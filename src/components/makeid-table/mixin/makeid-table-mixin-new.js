export default {
    ...
    beforeRouteEnter(to, from, next) {
        next((vm) => {
            if (from.name === "Dashboard") {
                ...
                vm.queryForm = vm.$route.query;
                console.log("vm.queryForm", vm.queryForm);
            }
            vm.mixinMounted ? vm.getDataList?.(1) : "";
        });
    },
    mounted() {
    ...
    methods: {
        ...
        addOrUpdateHandle(row) {
            ...
            this.$nextTick(() => {
                this.$refs["makeid-table-dialog"].tableDialogInit(row); //调用弹窗组件的初始化方法
            });
        },
